...
from isaca_api.slicap_adapter import _bode_frequency_range
...
def test_result_pages_are_offline_and_include_core_values() -> None:
    result = {
        ...
        "analyses": {"laplace": {"laplace": "1/(C*R*s + 1)", "_latex": {"laplace": r"\frac{1}{CRs+1}"}}, "pz": {"poles": [-1000.0], "zeros": [], "DCvalue": "1"}},
    }
...
def test_symbolic_worker_contract_keeps_root_errors_and_tex_separate() -> None:
    ...
    item = SimpleNamespace(
        ...
        target_root_approximations=(SimpleNamespace(expression=-1/(R*C), parameters=("R", "C"),
                                                  kind="pole", category="OLR-O", relative_root_error=0.01),),
    ...
    assert record["target_roots"][0]["relative_root_error"] == 0.01
